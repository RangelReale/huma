...
func TestRegisterHandle(t *testing.T) {
	...
	r.Handle("GET /x", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		handler(humago.NewContext(op, r, w))
	}))
	...
	// }
	// req, _ := http.NewRequest(feature.Method, feature.URL, body)
	req, _ := http.NewRequest(http.MethodGet, "/x", body)
	// for k, v := range feature.Headers {
	// 	req.Header.Set(k, v)
	// }
	w := httptest.NewRecorder()
